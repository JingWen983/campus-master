...
系统**首次启动且 `users` 表为空**时，会写入 4 个内置账号（`admin` / `teacher` / `student` / `parent`）。它们的**初始口令由 CSPRNG 随机生成**（每个账号独立、两两不同），**不存在固定的默认口令**。

| 角色 | 用户名 | 初始口令 |
|------|--------|----------|
| 管理员 | admin | 首启随机生成，仅向控制台打印一次 |
| 教师 | teacher | 首启随机生成，仅向控制台打印一次 |
| 学生 | student | 首启随机生成，仅向控制台打印一次 |
| 家长 | parent | 首启随机生成，仅向控制台打印一次 |

- 口令**只在首次启动时输出到控制台（标准输出）一次**，不写入任何文件（不会进 `server.log`）；请立即保存。
- 这些账号被标记 `must_change_password=1`：**首次登录必须改密**，改密成功前不能调用其它业务接口。
- **存量库**：旧版种子账号的无盐 SHA-256 记录、以及修复前遗留的 `pbkdf2$` 记录，一律视为**失效凭据**，需运维强制重置；登录时**没有**任何自动升级链路（不会"用一次就自动修好"），重置清单必须同时覆盖这两类记录。
...
2. 数据库文件应定期备份，避免数据丢失
3. 修改数据库结构前请先备份
4. 生产环境不存在可预测默认口令（内置账号在首启时随机生成），请妥善保存控制台打印的初始口令，并在首次登录后按系统强制要求完成改密